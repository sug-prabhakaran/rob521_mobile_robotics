...
TRANS_VEL_OPTS = [0, 0.025, 0.13, 0.20, 0.26]  # m/s, max of real robot is .26
...
class PathFollower():
    ...
    def follow_path(self):
        while not rospy.is_shutdown():
            ...
            for i, option in enumerate(self.all_opts):
                ...
                desired_head = np.around(np.arctan2((self.cur_goal[1]-self.pose_in_map_np[1]),
                                                       (self.cur_goal[0]-self.pose_in_map_np[0])), 4)
                actual_head = np.around(self.pose_in_map_np[2], 4)
                heading_error = np.around(self.normalize_angle(desired_head-actual_head), 4)

                # calculate what ideal vel, rot_vel would be according to simulation
                vel, rot_vel = self.ideal_controller(heading_error)
                ...
                print("heading error calc - desired:", desired_head, "actual:", actual_head, "error:", heading_error)
    ...
    def ideal_controller(self, err_head):
        ...
        # angular velocity = kP x err + kD x (err - prev_err)/delta_t
        rot_vel = np.round(self.kP*(err_head) + 
                  self.kD*(err_head - self.prev_err_head)/(self.delta_time) +
                  self.kI*(err_head + self.cumul_err_head)*self.delta_time, 2)
